...
print_usage() {
    ...
    echo "  first step (extracting annotations), load an anvio environment"
    echo "  Before running for the second step (decoding the annotations),"
    echo "  load the keggdecoder environment."
    echo ""
    echo " Step 1:"
...
print_version() {
    echo "Last modified 10 Sep 2026"
}
...
if [[ ! -e $all_kos || -n $reset ]]; then
    ...
    if [[ ! $CONDA_DEFAULT_ENV =~ "anvio" ]]; then
        echo "You need to load an anvio conda environment before running this step."
        echo ""
        exit 1
    fi
